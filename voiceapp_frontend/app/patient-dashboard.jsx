...
const API_URL = "http://10.0.2.2:5000/api/patientRequests"; // Use your actual local IP
...
const PatientDashboard = ({ setPage, user }) => {
  ...
  useEffect(() => {
    const fetchPastRequests = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/history/${user.user.patientID}`);
  ...
  const handleRequestSelect = async (requestText) => {
    ...
    try {
      await axios.post(`${API_URL}/submit`, { patientID: user.user.patientID, requestText });
